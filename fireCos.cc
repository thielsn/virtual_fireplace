...
static void DrawOnCanvas(Canvas *canvas) {
  ...
  for (int i = 0; i < 20000; ++i){
    ...
    for (int x = 0 ; x < WIDTH ; x++){
      ...
      int val1 = ( (cos (counter*seed1 + x * step + M_PI/4) + 1) / 2.0 ) * 126;
      int val2 = ( (cos (counter*seed2 + x * step*seed5) + 1) / 2.0 ) * 126;

      for (int y = 0 ; y < HEIGHT ; y++){

        int val3 = ( (cos (counter*seed3 + y * step + M_PI/4) + 1) / 2.0 ) * 126;
        int val4 = ( (cos (counter*seed4 + y * step*seed6) + 1) / 2.0 ) * 126;
