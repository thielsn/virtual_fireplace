...
static void DrawOnCanvas(Canvas *canvas) {
  ...
  for (int i = 0; i < 2000; ++i){
    ...
    for (int x = 0 ; x < WIDTH ; x++){
      ...
      int val1 = ( (sin (counter*seed1 + x * step + M_PI/4) + 1) / 2.0 ) * 80;
      int val2 = ( (sin (counter*seed2 + x * step*seed5) + 1) / 2.0 ) * 80;

      for (int y = 0 ; y < HEIGHT ; y++){

        int val3 = ( (sin (counter*seed3 + y * step + M_PI/4) + 1) / 2.0 ) * 35;
        int val4 = ( (sin (counter*seed4 + y * step*seed6) + 1) / 2.0 ) * 50;
